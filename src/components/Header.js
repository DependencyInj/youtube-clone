      ...
      <div className="grid grid-flow-col pl-10 pr-10 pt-5 pb-3 sticky top-0 bg-white">
        ...
        <div className="col-span-9 flex justify-center">
          <div className="flex w-[550px]">
            <input
              ref={searchInputRef}
              className="border px-3 border-gray-400 rounded-l-full w-4/6 p-1"
              type="text"
              value={searchQuery}
              onChange={(e) => {
                ...
                <ul ref={listRef}>
                  {suggestions.map((s) => (
                    <li
                      key={s}
                      id={"list-item" + s}
                      onClick={() => setSearchQueryInStore(s)}
                      className="py-2 px-3 cursor-pointer shadow-sm hover:bg-gray-200"
                    >
                      {" "}
                      🔍 {s}{" "}
                    </li>
